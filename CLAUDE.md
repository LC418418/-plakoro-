...
node tools/test-features.mjs     # 68 項：遺物上限／特訓／四天王補給／傷害尾巴／道具袋／
                                 #        敵人倒下免費換人／四天王換人／魔鬼難度封咗／
                                 #        **進化唔會換屬性同骰組**（掃齊 386 隻嘅進化鏈）
...
- **`cardOf(mon)` 一定要行 `baseDex`，唔可以行 `mon.dex`（v51 修好）。**
  招式同骰組係喺**基礎形態**生成（`mkPartyMon` 特登用 `baseDex`），進化之後
  淨係換外觀。但 `cardOf` 本來寫住 `mon.dex`，即係「進化完再攞張牌」會攞到
  **另一隻**嘅招式池同骰組 —— 學新招、晶片獎勵、換晶片之後重砌骰、
  骰子工房顯示同改裝，六個位一齊中招。
  真事：班基拉斯（幼基拉斯 闘 → 班基拉斯 悪）一入工房改一格骰，
  `buildDice` 就照住 D248 重砌，成套骰變晒悪，四張闘系招式即刻報廢，
  **唔會報錯**，玩家淨係見到「隻寵物突然出唔到招」。
- **進化唔可以換屬性（同上，v51）。** `evoSpec()` 一份三個地方用（即場進化／
  讀存檔／PvP 快照），`spec.type` 一律留返基礎形態嗰個。
  `deserializeRun` 同 PvP 快照本來仲會寫 `type:d.type`，即係「打機嗰陣顯示闘、
  reload 之後變咗悪」。386 隻入面得 **5 條鏈**會轉屬性：
  129→130（水→空）、204→205（草→鋼）、246→248（闘→悪）、
  290→292（草→悪）、328→329/330（闘→空）。
  ⚠ 弱點照跟進化後嗰隻，但**撞正等於自己屬性就唔換** —— 班基拉斯官方係
  「悪系弱闘」，而佢條鏈嘅基礎屬性正正就係闘，唔擋就變成「闘系弱闘系」。
  `test-features.mjs` 有一項掃齊 386 隻嘅進化鏈守住呢三樣。
